...
module.exports = {
    ...
    run: function(creep) {
        ...
        let target;
        if (creep.memory.hop) {
            let targets = creep.room.find(FIND_MY_CREEPS, { filter: (cr) => cr.hits < cr.hitsMax && cr.memory.role == "hopper" });
            target = targets.shift();
        } else {
            target = Game.creeps[creep.memory.target];
        }
        if(!target && creep.memory.targetType) {
            let targets = creep.room.find(FIND_MY_CREEPS, { filter: (cr) => cr.hits < cr.hitsMax && cr.memory.role == creep.memory.targetType });
