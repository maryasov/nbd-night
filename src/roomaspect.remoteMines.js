...
module.exports = class RemoteMinesAspect {
    ...
    spawnDefender(remoteRoom) {
        ...
        if(!hasDefender) {
            this.spawn(spawnHelper.bestAvailableParts(this.room, defender.closeConfigs()), { role: defender.name, room: remoteRoom.name, originRoom: this.room.name }, remoteRoom.name);
        }
    ...
    spawnReserver(remoteRoom) {
        ...
        var needReservation = !remoteRoom.controller.owner && (!remoteRoom.controller.reservation || remoteRoom.controller.reservation.ticksToEnd < 5000);
