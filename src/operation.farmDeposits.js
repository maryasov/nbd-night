...
module.exports = class FarmDepositsOperation {
  ...
  observe(roomName) {
    if (this.roomai.observer.isAvailable()) {
      ...
      !Game.rooms[roomName] &&
      !_.any(spawnHelper.globalCreepsWithRole(observer.name), (c) => c.memory.target == roomName)
    ) {
  ...
  spawnForDeposit(deposit) {
    ...
    for (let minerCreep of miners) {
      if (_.filter(carriers, (c) => c.memory.source == minerCreep.id).length < this.carrierCount) {
        let carrierParts = spawnHelper.makeParts(15, CARRY, 15, MOVE); // TODO: get smaller for high cooldown Deposits?
        let memory = {
          role: carrier.name,
    ...
    if (_.filter(miners, (c) => !c.ticksToLive || c.ticksToLive > timeToArrival).length < this.minerCount) {
      this.roomai.spawn(miner.depositParts, {
        role: miner.name,
        target: deposit.id,
        targetRoom: deposit.room.name,
