...
module.exports = class ConstructionSpaceFinder {
      ...
      for (let y = 0; y <= roomHeight; y++) {
        if (widthCache[y] > currentWidth) {
          ...
          do {
            ...
            if (
              (currentWidth >= minEdge1 && currentHeight >= minEdge2) ||
              (currentWidth >= minEdge2 && currentHeight >= minEdge1)
            ) {
  ...
  isFreeSpace(x, y) {
    return (
      x > 0 && x < 49 && y > 0 && y < 49 && this.terrain.get(x, y) !== TERRAIN_MASK_WALL && !this.buildProxy.get(x, y)
    );
  }
  ...
  selectBest(candidates) {
    let selected = [];
    for (let rect of _.sortBy(
      _.sortBy(candidates, (r) => -r.area),
      (r) => -Math.min(r.width, r.height)
    )) {
