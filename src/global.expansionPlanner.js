...
module.exports = class ExpansionPlanner {
  ...
  run() {
    if (!Memory.enableAutoExpansion) return;
    if (Game.time % 2000 !== 0) return;
    if (this.roomCount >= this.cpuRoomCap || this.roomCount >= this.gclRoomCap) return;

    let hasIncompleteClaimOperation = _.find(
      Operation.operations,
      (o) => o.type === 'claim' && !(o.spawnPosition.room && o.spawnPosition.room.controller.my)
    );
    ...
    Operation.createOperation('claim', {
      supportRoom: room.supportRoom,
      spawnPosition: new AbsolutePosition(new RoomPosition(25, 25, room.name)),
      autoPlanRoom: true,
      spawnBuilders: withBuilders,
    });
  }
  ...
  drawRoomScores() {
    for (let room of MapKnowledge.knowledgeList()) {
      ...
      Game.map.visual.text(score, new RoomPosition(42, 5, room.name), {
        fontSize: 8,
        color: '#ffffff',
        align: 'center',
        opacity: 1.0,
  ...
  get cpuRoomCap() {
    let cpuPerRoom =
      this.averageCpu / Math.max(_.filter(Game.rooms, (r) => r.ai() && r.controller.level >= 5).length, 1);
  ...
  get minerals() {
    if (!this._minerals) {
      let mineralAmounts = _.mapValues(
        _.groupBy(
          _.map(
            _.filter(Game.rooms, (r) => r.ai()),
            (r) => MapKnowledge.roomKnowledge(r).mineral
          ),
          (m) => m
        ),
        (rooms) => rooms.length
  ...
  selectNewRooms() {
    ...
    let supportRooms = _.map(
      _.filter(Game.rooms, (r) => r.ai() && r.controller.level >= minSupportLevel),
      (r) => r.name
    );
    ...
    claimables = _.sortBy(
      _.filter(claimables, (r) => r.supportDistance <= maximumSupportDistance),
      (r) => r.supportDistance
    );
