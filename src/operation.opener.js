...
module.exports = class AttackOperation extends Operation {
    ...
    run() {
        ...
        if(this.memory.terminateAfterSuccess && this.targetPosition.room) {
            ...
            if(this.targetPosition.room.find(FIND_STRUCTURES, {filter: (s) => prioritizedStructures.includes(s.structureType) && covered.filter((b)=>(b.x == s.pos.x && b.y == s.pos.y)).length > 0}).length === 0) {
                Operation.removeOperation(this);
                Operation.createOperation("scoop", { supportRoom: "W29S11", targetRoom: "W29S19", scooperCount: 15, terminateWhenEmpty: true });
            }
        }

        this.openers = _.filter(spawnHelper.globalCreepsWithRole(this.attackRole.name), (c) => c.memory.operation === this.id);
    ...
    drawVisuals() {
        ...
        if(targetPos) {
            ...
            RoomUI.forRoom(targetPos.roomName).addRoomCaption(`Opening from ${this.memory.supportRoom} with ${this.memory.openerCount} ${this.attackRole.name}s`);
            RoomUI.forRoom(this.memory.supportRoom).addRoomCaption(`Opening ${targetPos.roomName} with ${this.memory.openerCount} ${this.attackRole.name}s (next wave in ${this.memory.nextWaveAt - Game.time})`);
        }
    }
    ...
    spawnHealers(roomai) {
        ...
        for(let openerCreep of this.openers) {
            if(!_.any(healers, (c) => c.memory.target === openerCreep.name)) {
                ...
                let spawnResult = roomai.spawn(healerParts, { role: healer.name, target: openerCreep.name, operation: this.id });
                if(_.isString(spawnResult)) {
