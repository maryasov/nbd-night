...
const centerRow = [STRUCTURE_EXTENSION, STRUCTURE_EXTENSION, STRUCTURE_EXTENSION, STRUCTURE_ROAD];

const cornerRow = [STRUCTURE_EXTENSION, STRUCTURE_ROAD];
...
module.exports = {
  ...
  outline: function (room, cluster) {
    room.visual.rect(cluster.x - 0.5, cluster.y - 0.5, cluster.width, cluster.height, {
      stroke: '#77f',
      fill: null,
    });
  },
  ...
  plan: function (spaceFinder, buildings, room) {
    let missingExtensions =
      neededExtensions - _.sum(buildings, (b) => (b.type === this.type ? clusterSize(b.memory) : 0));
    ...
    for (let space of spaces) {
      ...
      cluster = {
        ...cluster,
        ...layout.alignInSpace(preferredPos, space, { width: cluster.width, height: cluster.height }),
      };
