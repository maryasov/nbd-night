...
module.exports = {
  ...
  outline: function (room, store) {
    ...
    if (store.link) {
      room.visual.poly(
        _.map(linkOutline, (p) => [x + p.x, y + p.y]),
        { stroke: color }
      );
    } else {
      room.visual.poly(
        _.map(containerOutline, (p) => [x + p.x, y + p.y]),
        { stroke: color }
      );
    }
  },
  ...
  plan: function (spaceFinder, buildings, room) {
    ...
    for (let source of room.find(FIND_SOURCES)) {
      ...
      eachSpaceAround(source, spaceFinder, (minerPos) =>
        eachSpaceAround({ pos: minerPos, room: room }, spaceFinder, (pos) =>
          result.push({ x: pos.x, y: pos.y, link: true })
        )
      );
    }
