...
module.exports = {
  ...
  build: function (proxy, exit) {
    ...
    eachWallPosition(exit, (pos) => {
      if (Memory.buildRamparts) {
        ...
        if (exitsInPosition) {
          ...
          const fWall = _.find(struct, (w) => w.structureType === 'constructedWall');
          if (fWall) {
            fWall.destroy();
          }
          // console.log('str', str)
  ...
  plan: function (spaceFinder, buildings, room) {
    let existingWalls = _.map(
      _.filter(buildings, (b) => b.type === this.type),
      (b) => b.memory
    );
    let result = [];
    let currentExit = {};
    for (let pos of room.find(FIND_EXIT)) {
      if (!extendExit(currentExit, pos)) {
        if (
          currentExit.edge &&
          !_.any(existingWalls, (w) => w.edge === currentExit.edge && w.start === currentExit.start)
        )
          result.push(currentExit);
    ...
    if (currentExit.edge && !_.any(existingWalls, (w) => w.edge === currentExit.edge && w.start === currentExit.start))
      result.push(currentExit);
