...
module.exports = class AttackOperation extends Operation {
  ...
  run() {
    ...
    if (this.memory.terminateAfterSuccess && this.targetPosition.room) {
      if (
        this.targetPosition.room.find(FIND_HOSTILE_STRUCTURES, {
          filter: (s) => keyStructures.includes(s.structureType),
        }).length === 0
      ) {
        Operation.removeOperation(this);
      }
  ...
  drawVisuals() {
    ...
    if (targetPos) {
      ...
      RoomUI.forRoom(targetPos.roomName).addRoomCaption(
        `Attacking from ${this.memory.supportRoom} with ${this.memory.attackerCount} ${this.attackRole.name}s`
      );
      RoomUI.forRoom(this.memory.supportRoom).addRoomCaption(
        `Attacking ${targetPos.roomName} with ${this.memory.attackerCount} ${this.attackRole.name}s (next wave in ${
