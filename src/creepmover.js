...
module.exports = class CreepMover {
    ...
    if (!CreepMover.samePos(data.target, target.pos)) {
      ...
      if (data.path) {
        ...
        if (CreepMover.sameCoord(this.creep.pos, expectedPos)) {
          ...
          CreepMover.sameCoord(this.creep.pos, CreepMover.coordInDirection(data.lastPos, this.nextDir(data)))
        ) {
    ...
    if (!data.path) {
      ...
      if (this.creep.pos.isNearTo(target)) {
        ...
        if (result.incomplete) {
          ...
          this.creep.memory.stuckCnt = (this.creep.memory.stuckCnt || 0) + 1;
        }
  ...
  rangeByTarget() {
    if (this.target.structureType && OBSTACLE_OBJECT_TYPES.includes(this.target.structureType)) {
      ...
      Game.map.getRoomTerrain(this.target.pos.roomName).get(this.target.pos.x, this.target.pos.y) === TERRAIN_MASK_WALL
    ) {
      return 1;
    }
