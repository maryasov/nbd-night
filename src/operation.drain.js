...
module.exports = class DrainOperation extends Operation {
  ...
  supportRoomCallback(room) {
    ...
    let hoppers = _.filter(
      spawnHelper.globalCreepsWithRole(hopper.name),
      (c) => c.memory.operation === this.id && (c.ticksToLive > this.creepRenewDuration(room) || !c.ticksToLive)
    );

    for (let hopperCreep of hoppers) {
      //console.log(`${hopperCreep.name} any ${!_.any(healers, (c) => c.memory.target === hopperCreep.name)}`);
      if (!_.any(healers, (c) => c.memory.target !== hopperCreep.name)) {
        ...
        if (hopperCreep.spawning) {
          ...
          healerParts = spawnHelper.bestAffordableParts(
            room,
            healer.configs({ minHeal: 5, maxHeal: 25, healRatio: 1 })
          );
        }
        roomai.spawn(healerParts, {
          role: healer.name,
          hop: true,
          target: hopperCreep.name,
          avoidRooms: [this.memory.targetRoom],
          operation: this.id,
        });
      }
    }
    ...
    if (hoppers.length < 1) {
      roomai.spawn(spawnHelper.bestAvailableParts(room, hopper.configs()), {
        role: hopper.name,
        room: this.memory.targetRoom,
        operation: this.id,
      });
    }
  }
  ...
  creepRenewDuration(startRoom) {
    ...
    //console.log(`${startRoom.name}, ${this.memory.targetRoom}`);
    let travelTime = Game.map.getRoomLinearDistance(startRoom.name, this.memory.targetRoom) * 50;
