...
module.exports = {
  ...
  configs: function () {
    ...
    for (let strength = 25; strength >= 5; strength -= 1) {
      ...
      let config = Array(strength - heal)
        .fill(RANGED_ATTACK)
        .concat(Array(strength).fill(MOVE))
        .concat(Array(heal).fill(HEAL));
  ...
  attackRoom: function (creep) {
    ...
    hostiles = _.sortBy(
      _.sortBy(hostiles, (h) => h.pos.getRangeTo(creep)),
      (h) => (_.some(h.body, (p) => p.type === ATTACK || p.type === RANGED_ATTACK) ? 0 : 1)
    );
