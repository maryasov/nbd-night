...
module.exports = class DowngradeOperation extends Operation {
  supportRoomCallback(room) {
    ...
    let downgraders = _.filter(
      spawnHelper.globalCreepsWithRole(downgrader.name),
      (c) => c.memory.operation === this.id
    );

    if (downgraders.length === 0) {
      roomai.spawn(spawnHelper.bestAvailableParts(roomai.room, downgrader.configs()), {
        role: downgrader.name,
