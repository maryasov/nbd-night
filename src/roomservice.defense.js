...
module.exports = class Defense {
  ...
  displayDefcon() {
    let defcon = this.memory.defcon;
    if (defcon.level === 0) return;
    RoomUI.forRoom(this.room).addRoomCaption(`Defcon ${defcon.level} (${defcon.progress} T; CD ${defcon.cooldown} T)`, {
      color: '#faa',
    });
  }
