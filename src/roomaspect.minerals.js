...
module.exports = class MineralsAspect {
  ...
  run() {
    //return; //Остановка добычи минерала
  ...
  buildCarrier() {
    ...
    if (
      mineralStore === this.room.terminal ||
      mineralStore === this.room.storage ||
      _.any(
        spawnHelper.localCreepsWithRole(this.roomai, carrier.name),
        (creep) => creep.memory.source == this.mineral.id
      )
    ) {
      return;
    }
  ...
  needWorkers() {
    if (this.roomai.canSpawn() && this.mineral.mineralAmount > 0) {
      if (
        this.roomai.trading.requiredExportFromRoom(this.mineral.mineralType, { showExcess: true }) <
        mineralExcessThreshold
      ) {
        return true;
      }
    }
