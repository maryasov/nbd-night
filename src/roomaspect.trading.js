...
module.exports = class TradingAspect {
    ...
    balanceToEmpire(resource, amount, cache) {
        ...
        if(choice) {
            ...
            if (status == 0 && sentAmount > transactionPrice) {console.log('balanceToEmpire', status, this.terminal.room.name, resource, sentAmount, transactionPrice, choice.room.name);
            } else {
