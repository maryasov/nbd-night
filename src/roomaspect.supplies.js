...
module.exports = class SuppliesAspect {
    ...
    buildHarvesters(source) {
        ...
        if (ignoreReserved) {memory.ignoreReserved = ignoreReserved}
        var res = this.roomai.spawn(parts, memory);
        // console.log('res h spawn', res)

        if (res !== 0 && numHarv === 0) {
            if (Memory.rooms[this.room.name].failBuildHarvester === undefined) { Memory.rooms[this.room.name].failBuildHarvester = 0 }
