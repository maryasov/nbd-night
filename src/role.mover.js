...
module.exports = {
    ...
    pickupWay: function(creep) {
        ...
        if (!target) target = creep.pos.findClosestByRange(FIND_TOMBSTONES, {filter: (t) => _.sum(t.store) > 0 && (t.creep && t.creep.my)});
    ...
    destination: function(creep) {
        ...
        // console.log('sc targ', target)
        targets = targets.concat(room.find(FIND_STRUCTURES, {filter: (s) => storeStructures.includes(s.structureType) && _.sum(s.store) < s.storeCapacity * 0.6}));
        if(targets.length > 0) {
            var targetsByDistance = _.sortBy(targets, (t) => creep.pos.getRangeTo(t));
            // console.log('dist', JSON.stringify(targetsByDistance))
            target = targetsByDistance[0];
