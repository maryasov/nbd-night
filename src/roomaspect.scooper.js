...
module.exports = class ScooperAspect {
    ...
    run() {
        ...
        let tombstones = this.room.find(FIND_TOMBSTONES, { filter: (t) => _.sum(t.store) > 0 }).concat(this.room.find(FIND_RUINS, { filter: (t) => _.sum(t.store) > 0 }));
        ...
        if (spawnPicker) {
            this.roomai.spawn(spawnHelper.bestAvailableParts(this.room, picker.configs(500)), { role: picker.name, home: this.room.name, target: this.room.name, renew: true });
        }
    }
}
