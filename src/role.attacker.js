...
module.exports = {
  ...
  attackRoom: function (creep, position) {
    ...
    if (!target) {
      target = creep.pos.findClosestByRange(FIND_HOSTILE_STRUCTURES, {
        filter: (s) => s.structureType != STRUCTURE_CONTROLLER && s.structureType != STRUCTURE_STORAGE,
      });
    }
  ...
  aggressiveMove: function (creep, target) {
    ...
    if (creep.memory._move.path) {
      ...
      if (!moveTarget) {
        moveTarget = _.find(creep.room.lookForAt(LOOK_CREEPS, creep.room.getPositionAt(nextStep.x, nextStep.y)), (c) =>
          ff.isHostile(c)
        );
      }
